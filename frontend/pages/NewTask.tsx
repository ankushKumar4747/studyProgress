...
const NewTask: React.FC = () => {
  ...
  const handleAISuggest = async () => {
    ...
    suggested.forEach(s => {
      if (!currentSubject.topics.some(t => t.title.toLowerCase() === s.toLowerCase())) {
        addTopicToSubject(s);
      }
    });
  };
    ...
    <div className="p-4 md:p-8 lg:p-12">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-3xl md:text-5xl font-black text-white tracking-tighter">Academic Planner</h1>
          <p className="text-[#94a3b8] mt-2 font-medium">Build your subjects and import your syllabuses.</p>
        </div>
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            <div className="bg-surface-dark border border-white/5 p-6 rounded-2xl shadow-xl flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-black text-[#64748b] uppercase tracking-widest">New Subject Name</label>
              ...
              <div className="flex flex-col gap-2 min-h-[100px] border-t border-slate-800 pt-4">
                <label className="text-xs font-black text-[#64748b] uppercase tracking-widest">Added Subjects</label>
            ...
            <div className="bg-surface-dark border border-white/5 p-6 md:p-8 rounded-2xl shadow-xl min-h-[500px] flex flex-col">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                  ...
                  <p className="text-sm text-[#94a3b8]">Select a subject to manage its curriculum.</p>
                </div>
              ...
              <div className="mb-6 p-4 bg-background-dark/50 border border-slate-800 rounded-2xl">
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black text-[#5a6b85] uppercase tracking-widest">Active Curriculum Context</label>
                  <select 
                    className="bg-transparent border-none text-white font-bold text-lg focus:ring-0 outline-none cursor-pointer p-0"
                    value={selectedSubjectId}
                    onChange={(e) => setSelectedSubjectId(e.target.value)}
                  >
                    <option value="" disabled className="bg-slate-900">Choose a subject from your list...</option>
                    {subjects.map(s => (
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-3">
                {!selectedSubjectId ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-center opacity-30 select-none">
                    ...
                    <p className="text-sm font-black uppercase tracking-widest">Select a Subject to start</p>
                    <p className="text-xs mt-1">Add subjects on the left to populate this view</p>
                  </div>
                ) : currentSubject?.topics.length === 0 ? (
                   <div className="flex-1 flex flex-col items-center justify-center text-center opacity-30 select-none">
                    <span className="material-symbols-outlined text-6xl mb-4">csv</span>
                    <p className="text-sm font-black uppercase tracking-widest">No Curriculum Found</p>
                    <p className="text-xs mt-1">Use AI Suggestion or Upload a CSV Syllabus</p>
                  </div>
                ) : (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-slate-500 uppercase font-black">Sub-task for {currentSubject.name}</span>
                        </div>
                      </div>
                      <button 
                        type="button"
